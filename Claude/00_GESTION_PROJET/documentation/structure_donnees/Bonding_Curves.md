...
3. **Pour notre vote**
   - Le classement se fait sur **NET score agrégé** (FOR - AGAINST) de tous les claims
   - Un même totem peut avoir **plusieurs claims** avec différents prédicats
   - Utiliser `aggregateTriplesByObject()` pour agréger les votes
   - Plus de NET $TRUST = totem plus populaire
